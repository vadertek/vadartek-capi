...
async function sendCAPIEvent({
  ...
  // If order older than 7 days → use current timestamp
  const safeEventTime =
  ...
  if (safeEventTime === now) {
    console.log(
      `⚠️ Order ${order.id} older than 7 days → using current timestamp for Meta`
    );
  }
...
module.exports = async (req, res) => {
  ...
  if (!PIXEL_ID || !ACCESS_TOKEN) {
    console.error(
      '❌ Missing CATDOG_FB_PIXEL_ID or CATDOG_FB_TOKEN'
    );

    return res
      .status(500)
      .json({ error: 'Pixel config missing' });
  }
  ...
  // Parse note_attributes into flat object
  const attrs = {};
  ...
  // Fetch product_type from Shopify Admin API
  ...
  const dogItems = items.filter(i =>
    isType(i.product_type, DOG_TYPE)
  );
